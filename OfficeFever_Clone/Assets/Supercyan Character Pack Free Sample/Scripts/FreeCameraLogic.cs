...
public class FreeCameraLogic : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        Quaternion currentRotation = Quaternion.Euler(0, currentRotationAngle, 270);

        Vector3 position = m_currentTarget.position;
        position -= currentRotation * Vector3.one * m_distance;
        position.y = targetHeight;
        transform.rotation = Quaternion.Euler(180,0,0);
        

        transform.position = position;
        transform.LookAt(m_currentTarget.position + new Vector3(0, m_height, 250));
    }
}
